...
- HAProxy control plane rules: a bad spec or failed `haproxy -c` must never replace the \
running config; backend-only changes must go through the runtime API without a reload.
